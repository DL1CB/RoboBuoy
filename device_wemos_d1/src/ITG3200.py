...
class ITG3200():
	...
	def power_configuration(self):
		...
		i2c.writeto_mem(ITG3200_DEFAULT_ADDRESS, ITG3200_PWR_MGM, bytearray([POWER_CONFIG]))
	...
	def fullscale_configuration(self):
		...
		i2c.writeto_mem(ITG3200_DEFAULT_ADDRESS, ITG3200_DLPF_PS, bytearray([FULLSCALE_CONFIG]))
	
	def read_gyro(self):
		"""Read data back from ITG3200_GYRO_XOUT_H(0x1D), 6 bytes
		X-Axis MSB, X-Axis LSB, Y-Axis MSB, Y-Axis LSB, Z-Axis MSB, Z-Axis LSB"""
		data = i2c.readfrom_mem(ITG3200_DEFAULT_ADDRESS, ITG3200_GYRO_XOUT_H, 6)
